...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Process TVQA data with CUDA and batch.')
    parser.add_argument('--train_file', type=str, required=True, help="Path to the TVQA train file.", default="./integrate_oricapkg_train.jsonl")
    parser.add_argument('--add_file', type=str, required=True, help="Path to the additional TVQA file.", default="./tvqa_train_naive.jsonl")
    parser.add_argument('--batch_size', type=int, default=8, help="Batch size for processing.")
    parser.add_argument('--output_file', type=str, default="./tvqa_train_naive_prob.jsonl", help="Output file for results.")
    parser.add_argument('--prompt_path', type=str, default="./tvqa_qa_gen_wo_qckg.txt")
    ...
    with open(args.add_file, 'r') as add_file:
        #count = 0
        #for add_line in add_file.readlines():
            #print(count)
            #additional_data = json.loads(add_line)
            #count = count + 1
        additional_data = [json.loads(add_line) for add_line in add_file.readlines()]
    ...
    for i, item in enumerate(train_data):
        ...
        question = item['q']
        ts = item['ts']
        ans_idx = item['answer_idx']
        show_name = item['show_name']
        captions = item['caption']
        #knowledge_graphs = item['kg']
        answer_num = "a"+str(ans_idx)
        answer = item[answer_num]
        items = json.dumps(item)
            
        #knowledge_graphs = json.dumps(knowledge_graphs)
        #srt = open('./subtitles/'+vid_name+'.srt', 'r')
        #subtitles = srt.read()
        prompt = prompt_format.format(items=items, question=question, answer=answer, captions=captions, vid_name=vid_name, 
                    ts=ts, show_name=show_name)
        text_in = prompt

        text_out_1 = ""
        for j in range(len(additional_data)):
            try:
                add_qid = additional_data[j]["qid"]
            except:
                import pdb; pdb.set_trace()
            add_qid = add_qid - 1
            if q_id != add_qid:
                ...
                try:
                    text_out_1 = additional_data[j]["q"]
                except:
                    import pdb; pdb.set_trace()
                dataset.append({"text_in": text_in, "text_out": text_out_1})
                dict_list.append(additional_data[j])


    # DataLoader 준비
    dataset = TVQADataset(dataset)
