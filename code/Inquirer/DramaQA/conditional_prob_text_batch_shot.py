...
if __name__ == "__main__":
    ...
    parser.add_argument('--add_file', type=str, required=True, help="Path to the additional DramaQA file.", default="./temp_data/AnotherMissOhQA_train_set_onlyadd_shot.json")
    parser.add_argument('--batch_size', type=int, default=8, help="Batch size for processing.")
    parser.add_argument('--output_file', type=str, default="./AnotherMissOhQA_train_set_add_sceneprob.json", help="Output file for results.")
    parser.add_argument('--prompt_path', type=str, default="./shotaddQA_20240618.txt")
    ...
    for i, (item, add_data) in enumerate(zip(train_data, additional_data)):
        ...
        shot_qas = item['qa']
        shot_kg = item['scene_graph']
            
        if args.speech:
            scripts = get_scripts(args)
            script = scripts[shot_id]
        prompt = prompt_format.format(shot_description=shot_description, knowledge_graph=shot_kg,
                                        shot_QA=shot_qas, script=script["subs"])
        text_in = prompt
