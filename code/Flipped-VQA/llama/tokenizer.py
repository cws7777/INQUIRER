...
class Tokenizer:
    ...
    def encode_kvqa(self, text=None, max_feats=10, split='train', answer_mapping=None, answer=None) -> List[int]:
        ...
        if split == 'train':
            s2 = s2 + answer_mapping[answer] 
            t2 = self.sp_model.encode(s2) + [self.eos_id]
            t = [t1 + [-2 for _ in range(max_feats)] + [self.nl_id] + k1 + t2]
        else:
            t = []
            for k, v in answer_mapping.items():
                t2 = self.sp_model.encode(s2 + v) + [self.eos_id]
                t.append(t1 + [-2 for _ in range(max_feats)] + [self.nl_id] + k1 + t2)
    ...
    def encode_kvaq(self, text=None, max_feats=10, split='train', answer_mapping=None, answer=None) -> List[int]:
        i_text = "Instruction: Predict the question based on the knowledge graph, video and answer.\n"
        q_text = text['q_text'].strip()
        o_text = text['o_text']
        a_text = text['a_text']
        k_text = str(text['k_text'])
    ...
    def encode_kqav(self, text=None, max_feats=10, max_seq_len=128, split='train', answer_mapping=None, answer=None) -> List[int]:
        i_text = "Instruction: Predict the video based on the dialogue, question and answer.\n"
        k_text = str(text['k_text'])
        q_text = text['q_text']
        o_text = text['o_text']
        a_text = text['a_text']
        s1, s2, s3 = i_text, k_text, q_text + o_text + a_text
