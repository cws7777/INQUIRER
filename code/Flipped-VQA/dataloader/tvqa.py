...
class TVQA(BaseDataset):
    def __init__(self, args=None, tokenizer=None, split='train'):
        ...
        if split == 'train':
            if args.naive == True:
                original_json_path = open(f'./data/tvqa/tvqa_{split}.jsonl', 'r')
                #oridata_list = list(original_json_path)
                original_data = [json.loads(x) for x in original_json_path.readlines()]
                
                additional_data_path = open(f'/home/wschoi/lba_dataset/lba_scene/generate_sceneqa/gen_tvqa/tvqa_{split}_add_prob.jsonl', 'r')
                #adddata_list = list(additional_data_path)
                additional_data = [json.loads(x) for x in additional_data_path.readlines()]
            ...
            original_json_path = f'./data/tvqa/tvqa_{split}.jsonl' 
            oridata_list = list(original_json_path)
            original_data = [json.loads(x) for x in oridata_list]
